...
export const CashDonationsPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [donRes, catRes] = await Promise.all([
        fetch(`/api/income/cash_donations?yearId=${activeYear.id}`, { headers }),
        fetch(`/api/master-data/income_categories?yearId=${activeYear.id}`, { headers })
      ]);
      
      setDonations(await donRes.json());
      setIncomeCategories((await catRes.json()).filter((c: any) => c.type === 'cash'));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const url = editingId 
        ? `/api/income/cash_donations/${editingId}`
        : `/api/income/cash_donations`;
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...formData, festival_year_id: activeYear.id, amount: Number(formData.amount) })
      });
      ...
      if (res.ok) {
        ...
        setEditingId(null);
        setFormData({ donor_name: '', amount: '', payment_method_name: '', category_name: '', donation_date: new Date().toISOString().split('T')[0], notes: '' });
        fetchData();
      } else {
  ...
  const handleEdit = (donation: any) => {
    ...
    setFormData({
      ...
      payment_method_name: donation.payment_methods?.name || '',
  ...
  const handleDelete = (id: string) => {
    setConfirmConfig({
      ...
      onConfirm: async () => {
        try {
          const { data: { session } } = await supabase.auth.getSession();
          const res = await fetch(`/api/income/cash_donations/${id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${session?.access_token}`,
              'X-Group-Id': activeGroupId || ''
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">{t('income.cashTitle')}</h2>
          <p className="text-muted-foreground">{t('income.cashDesc', { year: activeYear?.name || activeYear?.year || new Date().getFullYear() })}</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogTrigger asChild>
            <Button disabled={isLocked || activeGroupRole === 'viewer'}><Plus className="mr-2 h-4 w-4" /> {t('income.addCashDonation')}</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl">
            <DialogHeader><DialogTitle>{t('income.recordCash')}</DialogTitle></DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  {!isCustomCategory ? (
                    <select 
                      required 
                      className="flex h-12 w-full border-b border-brass/30 bg-surface px-4 py-2 text-base text-textPrimary focus-visible:outline-none focus-visible:border-brass transition-colors" 
                    ...
                    >
                      <option value="" disabled className="bg-surface text-textSecondary">{t('income.selectCategory') || 'Select Category...'}</option>
                      {Array.from(new Set(['General Donation', 'Annadhanam', 'Pooja', 'Abishekam', 'Vastra Dhanam', 'Archana', ...incomeCategories.map(c => c.name)])).map(name => (
                        <option key={name as string} value={name as string} className="bg-surface text-textPrimary">{name as string}</option>
                      ))}
                      <option value="ADD_NEW" className="bg-surface text-primary">+ Add New Category...</option>
                    </select>
                  ) : (
                ...
                <div className="space-y-2">
                  <Label>{t('income.paymentMethod')}</Label>
                  <select required className="flex h-12 w-full border-b border-brass/30 bg-surface px-4 py-2 text-base text-textPrimary focus-visible:outline-none focus-visible:border-brass transition-colors" value={formData.payment_method_name} onChange={e => setFormData({...formData, payment_method_name: e.target.value})}>
                    <option value="" disabled className="bg-surface text-textSecondary">{t('income.selectMethod') || 'Select Payment Method...'}</option>
                    <option value="Online" className="bg-surface text-textPrimary">Online</option>
                    <option value="In Hand" className="bg-surface text-textPrimary">In Hand</option>
                    <option value="Bank Transfer" className="bg-surface text-textPrimary">Bank Transfer</option>
                  </select>
                </div>
              </div>
              ...
              <Button type="submit" className="w-full">{t('income.submitDonation')}</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t('income.paymentMethod')}</TableHead>
            ...
            <TableBody>
              {donations.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground">{t('income.noCashFound')}</TableCell></TableRow>}
              {donations.map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell>{d.payment_methods?.name}</TableCell>
        ...
        <DialogContent className="max-w-sm text-center p-8 space-y-6 bg-ground border-white/10 shadow-2xl">
          ...
          <div className="bg-surface p-5 rounded-lg border border-white/5 space-y-3 shadow-inner">
            ...
            <div>
              ...
              <div className="text-4xl font-display font-bold text-positive drop-shadow-sm">₹{successData?.amount}</div>
            </div>
          </div>
