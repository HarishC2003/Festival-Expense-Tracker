...
export const GalleryPage: React.FC = () => {
  ...
  const fetchAlbums = async () => {
    ...
    const res = await fetch(`/api/media/albums?yearId=${activeYear.id}`, { 
      headers: { 
  ...
  const fetchItems = async (albumId: string) => {
    const { data: { session } } = await supabase.auth.getSession();
    const res = await fetch(`/api/media/gallery?albumId=${albumId}`, { 
      headers: { 
        'Authorization': `Bearer ${session?.access_token}`,
        'X-Group-Id': activeGroupId || ''
      } 
  ...
  const handleEditAlbumSubmit = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`/api/media/albums/${editAlbum.id}`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json', 
          'Authorization': `Bearer ${session?.access_token}`,
  ...
  const handleDeleteAlbum = async (id: string) => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`/api/media/albums/${id}`, {
        method: 'DELETE',
        headers: { 
          'Authorization': `Bearer ${session?.access_token}`,
          'X-Group-Id': activeGroupId || ''
